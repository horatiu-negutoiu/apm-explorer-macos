...
printf '%s\n' \
  ...
  'settings.hasPendingPermissionRecovery' \
...
printf '%s\n' \
  ...
  '<key>settings.hasCompletedLaunchAtLoginOnboarding</key><true/>' \
  '<key>settings.hasPendingPermissionRecovery</key><false/>' \
...
expect_success_count 8 "$verifier" --key-list "$allowed_keys"
expect_success_count 7 "$verifier" --plist "$valid_plist"
