...
public actor SQLiteActivitySessionRepository: ActivitySessionRepository {
  ...
  public func applyHourlyUpdates(_ updates: [HourlyActivityUpdate]) throws {
    ...
    do {
      try database.applyHourlyUpdates(updates)
    } catch let error as ActivitySessionStoreError {
      storedFailure = error
      throw error
    }
  ...
  public func openSession() throws -> ActivitySession? {
    try throwStoredFailure()
    let persisted = try database.openSession()
    return pendingSessions.values.first(where: \.isOpen) ?? persisted
  }
  ...
  private func runScheduledFlush() {
    ...
    do {
      ...
      storedFailure = .corruptSession(String(describing: error))
    }
  }

  private func flushPendingSessions() throws {
    guard !pendingSessions.isEmpty else { return }
    let sessions = Array(pendingSessions.values)
    do {
      try database.upsert(sessions)
