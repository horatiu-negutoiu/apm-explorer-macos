...
final class ActivitySignalIngestionExecutor: @unchecked Sendable {
    ...
    private var buffer: [RawActivitySignal] = []
    ...
    init(
        ...
        buffer.reserveCapacity(max(capacity, 1))
    }
    ...
    func setPersistenceFailureHandler(_ handler: @escaping @Sendable (String) -> Void) {
        queue.sync { persistenceFailureHandler = handler }
    }
    ...
    func checkpointMonitoring() {
        queue.sync {
            let wallNow = wallClock.now()
            let updates = monitoringUpdates(
                observedWallTime: wallNow,
                monotonicTime: monotonicClock.now(),
                countedAction: false
            )
            persist([], hourlyUpdates: updates)
            scheduleMaintenance(at: wallNow)
        }
    }
    ...
    private func persist(
        ...
        let failureHandler = persistenceFailureHandler
