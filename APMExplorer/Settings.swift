...
struct AppSettings: Equatable, Sendable {
    ...
    var hasCompletedLaunchAtLoginOnboarding = false
    var hasPendingPermissionRecovery = false
}
...
final class UserDefaultsSettingsRepository: AppSettingsPersisting, @unchecked Sendable {
    private enum Key {
        ...
        static let hasCompletedLaunchAtLoginOnboarding = "settings.hasCompletedLaunchAtLoginOnboarding"
        static let hasPendingPermissionRecovery = "settings.hasPendingPermissionRecovery"
    ...
    func load() -> AppSettings {
        ...
        settings.hasCompletedLaunchAtLoginOnboarding = defaults.bool(
            forKey: Key.hasCompletedLaunchAtLoginOnboarding
        )
        settings.hasPendingPermissionRecovery = defaults.bool(forKey: Key.hasPendingPermissionRecovery)
    ...
    private func saveUnlocked(_ settings: AppSettings) {
        ...
        defaults.set(
            settings.hasCompletedLaunchAtLoginOnboarding,
            forKey: Key.hasCompletedLaunchAtLoginOnboarding
        )
        defaults.set(settings.hasPendingPermissionRecovery, forKey: Key.hasPendingPermissionRecovery)
...
final class AppSettingsModel: ObservableObject {
    ...
    var hasCompletedLaunchAtLoginOnboarding: Bool {
        savedSettings.hasCompletedLaunchAtLoginOnboarding
    }

    func completeLaunchAtLoginOnboarding() {
        savedSettings.hasCompletedLaunchAtLoginOnboarding = true
        persist(actualLaunchState: launchAtLoginState)
    }

    func recordInputMonitoringPermission(_ permission: InputMonitoringPermissionState) {
        switch permission {
        case .notDetermined, .denied, .revoked, .relaunchRequired:
            guard !savedSettings.hasPendingPermissionRecovery else { return }
            savedSettings.hasPendingPermissionRecovery = true
        case .granted:
            guard savedSettings.hasPendingPermissionRecovery else { return }
            savedSettings.hasPendingPermissionRecovery = false
            savedSettings.hasCompletedLaunchAtLoginOnboarding = false
    ...
    private func persist(actualLaunchState: LaunchAtLoginState) {
        savedSettings.launchAtLogin = actualLaunchState == .on
